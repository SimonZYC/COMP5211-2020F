...
GHOST_COLORS = []
GHOST_COLORS.append(formatColor(219/255.0, 68/255.0, 55/255.0)) # Red
GHOST_COLORS.append(formatColor(66/255.0, 133/255.0, 244/255.0)) # Blue
GHOST_COLORS.append(formatColor(.98,.41,.07)) # Orange
GHOST_COLORS.append(formatColor(.1,.75,.7)) # Green
GHOST_COLORS.append(formatColor(1.0,0.6,0.0)) # Yellow
GHOST_COLORS.append(formatColor(.4,0.13,0.91)) # Purple
WALL_COLOR=formatColor(244/255.0, 180/255.0, 0)
TEAM_COLORS = GHOST_COLORS
...
class InfoPane:
  ...
  def drawPane(self):
    self.scoreText = text( self.toScreen(0, 0  ), self.textColor, self._infoString([0,0],1200), "Consolas", self.fontSize, "bold")
    self.redText = text( self.toScreen(0, 20  ), TEAM_COLORS[0], self._redScoreString(), "Consolas", self.fontSize, "bold")
    self.blueText = text( self.toScreen(0, 40  ), TEAM_COLORS[1], self._blueScoreString(), "Consolas", self.fontSize, "bold")
  ...
  def _infoString(self, score, timeleft):
    scoreString="SCORE: "+"".join(["%.2f vs "%s for s in score])
...
class PacmanGraphics:
  def __init__(self, redTeam, blueTeam, zoom=1.0, frameTime=0.0, capture=False):
    ...
    self.zoom = zoom
    self.gridSize = DEFAULT_GRID_SIZE * zoom *1.5
  ...
  def initialize(self, state, isBlue = False):
    ...
    self.nAgents=len(state.agentStates)
    global TEAM_COLORS
    TEAM_COLORS*=(self.nAgents//len(TEAM_COLORS)+1)
    global GHOST_COLORS
    GHOST_COLORS*=(self.nAgents//len(GHOST_COLORS)+1)
  ...
  def drawStaticObjects(self, state):
    ...
    self.food = self.drawFood(layout.food, state.foodPositions)
  ...
  def drawWalls(self, wallMatrix):
    ...
    for xNum, x in enumerate(wallMatrix):
      if self.capture and (xNum * 2) < wallMatrix.width: wallColor = WALL_COLOR # TEAM_COLORS[0]
      if self.capture and (xNum * 2) >= wallMatrix.width: wallColor = WALL_COLOR # TEAM_COLORS[1]
  ...
  def drawFood(self, foodMatrix, foodPositions):
    foodPos=[pos for (pos,eaten) in foodPositions]
    foodNotEaten=[pos for (pos,eaten) in foodPositions if not eaten]
    foodImages = [ [None for i in range(foodMatrix.height)] for i in range(foodMatrix.width) ]
    color = FOOD_COLOR
    for food in foodNotEaten:
          screen = self.to_screen(food)
          color=TEAM_COLORS[foodPos.index(food)]
          dot = circle( screen,
                        FOOD_SIZE * self.gridSize,
                        outlineColor = color, fillColor = color,
                        width = 1)  
          foodImages[int(food[0])][int(food[1])]=dot
